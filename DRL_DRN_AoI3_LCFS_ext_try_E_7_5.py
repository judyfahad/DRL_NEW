...
from modelGradients import modelGradients
...
for pres in range(1, 9):
    ...
    for i in range(episodes):
        ...
        for j in range(time_steps):
            ...
            if count >= M:
                minibatch = memSamp.DRL_Memory_Sample2(count, N, M, memory)
                S = np.array([minibatch[i][0] for i in range(M)])  # All current states from minibatch
                A = np.array([minibatch[i][1] for i in range(M)])  # Actions taken
                R = np.array([minibatch[i][2] for i in range(M)])  # Rewards received
                S_Next = np.array([minibatch[i][3] for i in range(M)])  # Next states
                # Convert states and next states to TensorFlow tensors
                ...
                R_current = policy_net(S_dlarray)  # R for current states
                R_next = target_net(S_Next_dlarray)  # R for next states

                # Compute the Target (the max Q-value for the next state across all actions)
                # Here, np.max is used to get the maximum value for each row (over actions).
                max_R_next = np.max(R_next.numpy(), axis=1)  # Shape (64,)
                Target = R.flatten() - estimate_average_reward + max_R_next  # Shape (64,)
                # Ensure actions are within the valid range
                A = np.clip(A, 0, actions_no - 1)
                R_target = R_current.numpy()  # Convert to NumPy for easier manipulation

                # Debug prints to verify shapes
                print(f"Shape of R_target: {R_target.shape}")  # Should be (64, 16)
                print(f"Actions (A): {A}")  # Ensure that values are within the range [0, 15]

                # Now perform the update
                R_target[np.arange(M), A] = Target  # Update target for the selected action in each state
                
                # Ensure A is flattened and use it to update the corresponding Q-values in R_target
                #A = np.array(A).flatten()
                #R_target = R_current.numpy()  # Convert to NumPy for easier manipulation

                # Final predicted R, based on the selected actions
                #print(f"Shape of Target: {Target.shape}")  # Should be (64,)
                #R_target[np.arange(M), A] = Target  # Update target for the selected action in each state
                ...
                gradients, loss = modelGradients(policy_net, S_dlarray, R_target)

                # Apply the Adam update
                #policy_net= adamupdate(policy_net, gradients, averageGrad, averageSqGrad, count - M + 1, lr)
